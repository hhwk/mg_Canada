...
if menu=='Гуманитарная помощь':
    ...
    visit_money = st.selectbox('Кому вы хотите перевести деньги?',('Мексика', 'Австралии', 'Филипинские острова', 'Швеция', 'Аргентина'))
...
if menu=='Посещения':
    for ii in range(0, 20):
        if request.get(f'{ii}') == None:
            break
    visit = st.selectbox('Какую старану вы хотите посетить?', ('Мексика', 'Австралию', 'Филипинские острова', 'Швеция', 'Аргентина'))
...
if menu=='Запуск ракет':
    ...
    if city['reserch']=='0':
        ...
        country = st.multiselect('Какие страны атакуем?',['Мексика', 'Австралию', 'Филипинские острова', 'Швеция', 'Аргентина'])
        for l in range(0,len(country)):
            if country[l]=='Мексика':
                attak=st.multiselect('Какие города атакуем в Мексика?',['Мехико','Канкун','Мерида','Таско'])
            if country[l]=='Австралию':
                attak1=st.multiselect('Какие города атакуем в Австралию?',['Кенберра','Сидней','Мельбрун','Перт'])
