...
if menu=='Гуманитарная помощь':
    ...
    visit_money = st.selectbox('Кому вы хотите перевести деньги?',('Мексике', 'Австралии', 'Филипинским островам', 'Швеции', 'Аргентине'))
...
if menu=='Посещения':
    visit = st.selectbox('Какую старану вы хотите посетить?', ('Мексика', 'Австралия', 'Филипинские острова', 'Швеция', 'Аргентина'))
...
if menu=='Запуск ракет':
    ...
    if city['reserch']=='0':
        ...
        country = st.multiselect('Какие страны атакуем?',['Мексика', 'Австралия', 'Филипинские острова', 'Швеция', 'Аргентина'])
        for l in range(0,len(country)):
            ...
            if country[l]=='Австралия':
                attak1=st.multiselect('Какие города атакуем в Австралия?',['Канберра','Сидней','Мельбрун','Перт'])
        ...
        if st.button('Отправить данные'):
            if final_roket>=0:
                ...
                Attak.put({'key':f'{ii}','Country':'Мексика'+str(attak),'Country1':'Австралия'+str(attak1),'Country2':'Филипинские острова'+str(attak2),'Country3':'Швеция'+str(attak3),'Country4':'Аргентина'+str(attak4)})
...
if menu=='Улучшения':
    ...
    sunks_for_who = st.multiselect('На какие страны вы хотите наложить санкции?', ['Мексика', 'Австралия', 'Филипинские острова', 'Швеция', 'Аргентина'])
    ...
    col2.metric('🏚️'+city['shit2']+masiv_shit[1]+f'{city_2}','⚙️'+str(0)+'%'+' 🌳 '+str(0)+'%',masiv_up[1]*10)
    col3.metric('🏚️'+city['shit3']+masiv_shit[2]+f'{city_3}','⚙️'+str(0)+'%'+' 🌳 '+str(0)+'%',masiv_up[2]*10)
...
if menu=='Стартовая страница':
    st.title(f'Вы играете за Канаду')
    st.write('Деньги:', money)
    ...
    col2.metric('🏚️' + city['shit2'] + f'{city_2}','⚙️' + str(0) + '%' + ' 🌳 ' + str(0) + '%')
    col3.metric('🏚️' + city['shit3'] + f'{city_3}','⚙️' + str(0) + '%' + ' 🌳 ' + str(0) + '%')
